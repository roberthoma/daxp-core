...
import static org.daxprotocol.core.test.dax_03_integration_test.dax_40_00_data_model.DaxAnySchemaRegister.*;



/*
*
* // 1. Dziedziczenie po klasie bazowej z własnymi polami DAXP
public class DaxAnyTestEntity extends AbstractDaxParentEntity {

    // 2. Ekstremalnie zagnieżdżona struktura generyczna
    @DaxpField(tagId = 9991)
    private Map<String, List<Map<Integer, Set<TestInsideRecord>>>> ultraNestedMap;

    // 3. Tablica wielowymiarowa obiektów DAXP
    @DaxpField(tagId = 9992)
    public DaxSubEntity[][] subEntityMatrix;

    // 4. Pole statyczne z adnotacją (konflikt z konwencją)
    @DaxpField(tagId = 9993)
    public static String STATIC_FIELD_TEST = "SHOULD_IGNORE_OR_HANDLE?";

    // 5. Polimorficzna lista obiektów abstrakcyjnych
    @DaxpField(tagId = 9994)
    public List<AbstractDaxParentEntity> polymorphicList;

    // 6. Typy czasu i identyfikatorów
    @DaxpField(tagId = 9995)
    public java.util.UUID entityUuid = java.util.UUID.randomUUID();

    @DaxpField(tagId = 9996)
    public java.time.ZonedDateTime timestamp = java.time.ZonedDateTime.now();
}
* */
...
public class DaxAnyTestEntity {   //TODO

    @DaxpCollection(tagId = 3000)
    ...
    @DaxpEntity(tagId = 9000)
    public record TestInsideRecord(
            @DaxpField(tagId = 9001) String test,
            @DaxpField(tagId = 9002) int count
    ) {}


    @DaxpEntity(tagId = 10000)
    public static class TestInsideClass{
        @DaxpField(tagId = 10002) public String test;

    }

    TestInsideClass testInsideClass = new TestInsideClass();

    @DaxpField(tagId = 3001)
    TestInsideEnum testInsideEnumValue = TestInsideEnum.INSIDE_ENUM_VAL1;
