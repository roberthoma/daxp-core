...
public class DaxClassRegisterService {
    ...
    public DaxClassRegisterService(DaxSemanticRegistry semanticRegistry,
        DaxTagCodec tagCodec,
        DaxDataTypeCodec dataTypeCodec
    ...
    public void registerByNote(DaxAnnotationNote annNote,
        ...
        semanticRegistry.putTag(annNote.getTag(), source, destiny);
        ...
        {
            System.out.println(">>> TAG TYPE >>>"+annNote.getDaxDataType() );
            semanticRegistry.putTagAtrDataType(annNote.getTag(),annNote.getDaxDataType());
        }
        else {
            System.out.println(">>> test TYPE >>>"+annNote.getDaxDataType() );
       //     throw new RuntimeException("DAX DATA TYPE UN........");
        ...
        if (destiny.equals(DaxTagDestiny.ENTITY_FIELD) ||
            ...
            semanticRegistry.putEntityField( annNote.getEntityTag(),annNote.getTag());
            semanticRegistry.putEntityEntryAtrName(annNote.getEntityTag(), annNote.getTag(), annNote.getName());
            semanticRegistry.putEntityEntryAtrDescription(annNote.getEntityTag(),annNote.getTag(), annNote.getDescription());

            if(annNote.isReadOnly()){
                semanticRegistry.putEntityEntryAtrReadOnly(annNote.getEntityTag(),annNote.getTag(), true);
            }

            if (annNote.isDeprecated()){
               semanticRegistry.putEntityEntryAtrDeprecated(annNote.getEntityTag(),annNote.getTag());
            }
        ...
        if (destiny.equals(DaxTagDestiny.TAG) ){
            if(annNote.isReadOnly()){
                semanticRegistry.putTagAtrReadOnly(annNote.getTag(), true);
            }


            semanticRegistry.putTagAtrDescription(annNote.getTag(), annNote.getDescription() );

        }

        if (destiny.equals(DaxTagDestiny.TAG) || destiny.equals(DaxTagDestiny.ENTITY)){
            if (annNote.isDeprecated()){
                semanticRegistry.putTagAtrDeprecated(annNote.getTag());
            }
        }
//---------------
